...
\title{No Optical Lens Can Fix Color-Blindness}
\author{J.L.}
\date{\today}
...
\begin{abstract}
This paper investigates the fundamental limitations of using optical lenses to correct color-blindness. We approach this problem by modelling the vision system, which includes the lens and the human eye, as a mixed quantum system. Through a mathematical proof, we demonstrate that color blindness, which can be represented as an illness\footnote{Pun intended.} in the color perception matrix, cannot be rectified by any unitary transformation corresponding to an optical lens. Our findings have implications for the design of corrective devices and challenge common practices in the field of vision science.
\end{abstract}
...
\]
where \( \psi \) is the light, \( \hbar \) is the reduced Planck's constant, and \( H \) is a \emph{Hermitian} operator representing the lens.
...
\end{eqnarray}
\begin{lem}
$U$ is unitary if and only if $H$ is Hermitian (which it is). 
\end{lem}

\subsection{Linear Transformation as Human vision}
The vision system is a process that converts light to intensity of response of photoreceptors, and there are 3 of them for a normal human. 
\begin{defn}
\begin{equation}
V:\Psi \rightarrow \Real^3
\end{equation}
\end{defn}
...
A normal vision system perceives color $\vec{c}= (r,g,b)$,
\begin{equation}
\vec{c} = V \psi 
\end{equation}

\begin{defn}
A normal color vision is characterized by 
\begin{equation}
\rank(V)=3
\end{equation}
\end{defn}
...
\begin{defn}
A fully-color-blind vision is characterized by 
\begin{equation}
\rank(V)<3
\end{equation}
or equivalently
...
\det(V V^\dagger) = 0
\end{equation}
and weakly-color-blind vision is characterized by 
\begin{equation}
\det(V V^\dagger) \approx 0
\end{equation}
\end{defn}

\begin{lem}
With an empirically obtained value $\eps$, both fully-color-blind and weakly-color-blind vision are characterized by 
\begin{equation}
\abs{\det(V V^\dagger) } < \eps
\end{equation}
\end{lem}
...
V' V'^\dagger = VU (VU)^\dagger 
                      =  VU U^\dagger V^\dagger 
                      =  V V^\dagger 
\end{eqnarray}

\begin{thm}
\begin{equation}
\abs{\det(V V^\dagger) } < \eps \iff  \abs{\det(V' V'^\dagger) } < \eps
\end{equation}
Hence, for any optical lens used to correct the vision, a color-blind vision will remain color blind.
\end{thm}
...
Future research should explore non-optical methods for addressing color-blindness. Potential avenues include genetic therapies aimed at modifying photoreceptor functionality or advanced neural prosthetics that can bypass the deficient color channels in the eye. Additionally, the exploration of non-linear optics or quantum computing techniques might offer new perspectives on the problem of correcting color vision deficiencies.
...
\section{Conclusion}
This paper demonstrates that color-blindness cannot be corrected by any optical lens, as such lenses are represented by unitary transformations that preserve the rank of the color perception matrix. Our findings suggest that alternative approaches, potentially involving non-optical methods, are necessary for addressing color-blindness. Future work may explore the implications of this result for the design of visual aids and for our understanding of the limits of optical correction.
